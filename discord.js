...
bot.on("messageCreate", (message) => {
    ...
    if (message.content.toLowerCase().startsWith(",coins ")) {
        //console.log("yeeet");
        const [CMD_NAME, ...args] = message.content.trim().substring(6).split(" ");
        exportedBot.emit("coins", message, message.channel.id, args[0], args[1]);
        
    }
    if (message.content.toLowerCase().startsWith(",remove_coins ")) {
        //console.log("yeeet");
        const [CMD_NAME, ...args] = message.content.trim().substring(13).split(" ");
        exportedBot.emit("remove_coins", message, message.channel.id, args[0], args[1]);
        
    }
    if (message.content.toLowerCase().startsWith(",bottrades ")) {
        //console.log("yeeet");
        const [CMD_NAME, ...args] = message.content.trim().substring(10).split(" ");
        exportedBot.emit("bt", message, message.channel.id, args[0], args[1]);
        
    }
    if (message.content.toLowerCase().startsWith(",remove_bottrades ")) {
        //console.log("yeeet");
        const [CMD_NAME, ...args] = message.content.trim().substring(17).split(" ");
        exportedBot.emit("remove_bt", message, message.channel.id, args[0], args[1]);
        
    }
    if (message.content.toLowerCase().startsWith(",auction_bottrades ")) {
        //console.log("yeeet");
        const [CMD_NAME, ...args] = message.content.trim().substring(18).split(" ");
        exportedBot.emit("auction_bottrades", message, args[0], args[1], args[2]);
        
    }
    //if (message.content.toLowerCase() === "test123") {
        //console.log("yeeet");
        //bot.channels.cache.get("1088181534676242482").send("Funktioniert!");
        //message.reply("Finally");
...
const dailyspinCommand = {
    name: "dailyspin",
    description: "Win coins, bottrades, packs or cards!",
    type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND
};
...
const forceEndTransactionMeCommand = {
    ...
    description: "[owner ONLY] :warning: Force ends your transaction :warning:",
    type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND
};
...
const setPacksCommand = {
    ...
    options: [
        {
            name: "type",
            description: "The type of bottrades where packs will be set",
            type: Constants.ApplicationCommandOptionTypes.STRING,
            required: true,
            choices: [
                {
...
const botCode = {
    name: "code",
    description: "Creates a Bot Code",
    type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND,
    options: [
        {
            name: "codename",
...
const premiumSpin = {
    name: "ultimate-spin",
    description: "Spin for good rewards!",
    type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND
};
...
const bottradesCommand = {
    name: "bottrades",
    description: "Use bot trades from your wallet",
    type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND,
    options: [
        {
            name: "amount",
            description: "The amount of bottrades send to madfut",
            type: Constants.ApplicationCommandOptionTypes.INTEGER,
            required: true
        },
        {
            name: "type",
            description: "Do you want packs or coins in bottrades",
            type: Constants.ApplicationCommandOptionTypes.STRING,
            required: true,
            choices: [
                {
...
const payCommand = {
    ...
    options: [
        ...
        {
            name: "ultimatespins",
            description: "The amount of Ultimate Spins to pay to the other user",
            type: Constants.ApplicationCommandOptionTypes.INTEGER,
            required: false
        },
    ]
...
const tradeCommand = {
    ...
    options: [
        ...
        {
            name: "giveultimatespins",
            description: "The amount of Ultimate Spins you want to give",
            type: Constants.ApplicationCommandOptionTypes.INTEGER,
            required: false
        },
        {
        ...
        {
            name: "receiveultimatespins",
            description: "The amount of Ultimate Spins you want to receive",
            type: Constants.ApplicationCommandOptionTypes.INTEGER,
            required: false
        },
    ]
...
const forceEndTransactionCommand = {
    ...
    description: "[owner ONLY] ⚠️ Force ends a user's transaction ⚠️",
    type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND,
    options: [
        {
            name: "user",
...
const inUseCommand = {
    name: "inUse",
    description: "[owner ONLY] ⚠️ See how many accounts of bot are in use! ⚠️",
    type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND
};
...
const adminRemoveCommand = {
    ...
    options: [
        ...
        {
            name: "ultimatespins",
            description: "The amount of Ultimate Spins to remove from the other user",
            type: Constants.ApplicationCommandOptionTypes.INTEGER,
            required: false
        }
    ]
...
const ownerCommand = {
    name: "owner",
    description: "[owner ONLY] All owner commands",
    type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND_GROUP,
    options: [
        ...giveawayCommand,
        adminRemoveCommand,
...
const vipCommand = {
    name: "vip",
    description: "[VIP ONLY] All vip commands",
    type: Constants.ApplicationCommandOptionTypes.SUB_COMMAND_GROUP,
    options: [
        sendCommand
    ]
...
function handleAdminCommand(interaction) {
    if (!interaction.member?.roles.includes(config.abuserRoleId)) {
    ...
    switch(cmdName){
        ...
        case 'nuke':
            ...
            const removeUltimateSpins = subsubcmd.options.find((option)=>option.name === 'ultimatespins'
            )?.value ?? 0;
            ...
            exportedBot.emit("remove", interaction, userremove, removeCoins, removeCards, removePacks, removeBotTrades, removeUltimateSpins);
            break;
        case "invite":
        ...
        case 'send':
            {
                if (!interaction.member.roles.includes(config.ownerRoleId.toString())) {
                    interaction.createMessage({
        ...
        case 'pay':
            ...
            const payingUltimateSpins = subsubcmd.options.find((option)=>option.name === 'ultimatespins'
            )?.value ?? 0;
            ...
            if (payingCoins === 0 && payingCards.length === 0 && payingPacks.length === 0 && payingBotTrades === 0 && payingUltimateSpins === 0) {
                interaction.createMessage("Input at least 1 item to pay.");
                break;
            }
            exportedBot.emit("admin-pay", interaction, user, payingCoins, payingCards, payingPacks, payingBotTrades, payingUltimateSpins);
            break;
        default:
            interaction.createMessage({
...
function handleownerCommand(interaction) {
    if (!interaction.member.roles.includes(config.ownerRoleId)) {
        interaction.createMessage({
    ...
    switch(cmdName){
        ...
        case "force-end-transaction":
            ...
            break;
        case 'setpacks':
            const packsabc = subcommand.options.find((option)=>option.name === 'packs'
            )?.value ?? "";
            const type = subcommand.options.find((option)=>option.name === 'type'
            )?.value ?? "";
            exportedBot.emit("setpacks", interaction, packsabc, type);
            break;
        //case 'inUse':
        //    exportedBot.emit("inUse", interaction);
        //    break;
        case 'giveaway':
            exportedBot.emit("ga-announce", interaction, "0.2", "1");
            break;
        ...
        case 'code':
            ...
            exportedBot.emit("code", interaction, codename);
            break;
        default:
            interaction.createMessage({
                content: `Unknown subcommand.`,
...
function handleVipCommand(interaction) {
    if (!interaction.member.roles.includes(config.vipRoleId)) {
        interaction.createMessage({
            content: `Only real vips can use this command.`,
            flags: Constants.MessageFlags.EPHEMERAL
        });
        return;
    }
    ...
    switch(cmdName){
        case 'send':
            {
                if (!interaction.member.roles.includes(config.vipRoleId.toString())) {
                    interaction.createMessage({
                        content: `Only real vips can use this command.`,
                        flags: Constants.MessageFlags.EPHEMERAL
                    });
    ...
    if (interaction instanceof CommandInteraction) {
        ...
        switch(subcommand.name){
            ...
            case 'admin':
                ...
                break;
            case 'owner':
                handleownerCommand(interaction);
                break;
            case 'vip':
                handleVipCommand(interaction);
                break;
            case 'claim':
            ...
            case 'bottrades':
                if (!Adminchannel.includes(interaction.channel.id)) {
                    interaction.createFollowup({
                        ...
                        {
                            color: 15158332,
                            description: "You cant run that command in this channel"
                        }
                        ]
                    });
                    break;
                }
                const bottrades = subcommand.options.find((option)=>option.name === 'amount'
                )?.value ?? 0;
                const type = subcommand.options.find((option)=>option.name === 'type'
                )?.value ?? "";
               
                exportedBot.emit("bottrades", interaction, bottrades, type);
                break;
            case 'pay':
                {
                    ...
                    const payingUltimateSpins = subcommand.options.find((option)=>option.name === 'ultimatespins'
                    )?.value ?? 0;
                    ...
                    exportedBot.emit("pay", interaction, user, payingCoins, payingCards, payingPacks, payingBotTrades, payingUltimateSpins);
                    break;
                }
            case "force-end-transaction-me":
                ...
                break;
            case "ultimate-spin":
            	  if (interaction.channel.id !== "1109253802105766008") {
                        interaction.createMessage({
                            content: `You can only use this command in the <#${1109253802105766008}> channel.`,
                            flags: Constants.MessageFlags.EPHEMERAL
                        });
                        break;
                    }
                exportedBot.emit("ultimate-spin", interaction);
                break;
            case 'trade':
                {
                    ...
                    const givingUltimateSpins = subcommand.options.find((option)=>option.name === 'giveultimatespins'
                    )?.value ?? 0;
                    ...
                    if (givingCoins === 0 && givingCards.length === 0 && givingPacks.length === 0 && givingBotTrades === 0 && givingUltimateSpins === 0) {
                    ...
                    const receivingUltimateSpins = subcommand.options.find((option)=>option.name === 'receiveultimatespins'
                    )?.value ?? 0;
                    ...
                    if (receivingCoins === 0 && receivingCards.length === 0 && receivingPacks.length === 0 && receivingBotTrades === 0 && receivingUltimateSpins === 0) {
                        interaction.createMessage("Input at least 1 item to receive.");
                        break;
                    }
                    exportedBot.emit("trade", interaction, user, givingCoins, givingCards, givingPacks, givingBotTrades, givingUltimateSpins, receivingCoins, receivingCards, receivingPacks, receivingBotTrades,  receivingUltimateSpins);
                    break;
                }
            case 'flip':
