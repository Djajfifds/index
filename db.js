...
try {
    await runPromise(`CREATE TABLE IF NOT EXISTS users (
		...
		ultimatespins INTERGER NOT NULL DEFAULT 0,
...
try {
    await runPromise(`CREATE TABLE IF NOT EXISTS packs (
		bottradepacks TEXT,
		botcodepacks TEXT,
		giveawaypacks TEXT
	);`);
} catch (err2) {
    console.log("Couldn't initialize database (creating packs table). Exiting");
    process.exit(1);
}
try {
    await runPromise(`CREATE TABLE IF NOT EXISTS rewards (
		id sqlite_int64 PRIMARY KEY,
		gold TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		totw TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		tots TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		icon TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		toty TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		madfut_icon TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		future_icon TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
...
try {
    await runPromise(`CREATE TABLE IF NOT EXISTS matchstatus (
		teamstatus TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		claimstatus TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		votestatus TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		votetime INTERGER NOT NULL DEFAULT 0,
		homestatus TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		drawstatus TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
		awaystatus TEXT NOT NULL DEFAULT 'Ã¢ÂÅ’',
...
function packToName(packQuery) {
    if (packQuery.startsWith("query,")) {
        try {
            ...
            if (color === "dscÃ¢â‚¬Â¤gg  ÃŒÂ¸madfutÃ¢Å½Â½Ã¢â‚¬â€°maestro | @ph0t0shop modded") {
...
async function getUltimateSpins(user) {
    const res1 = await getPromise("SELECT ultimatespins FROM users WHERE id = (?);", user);
    return res1?.ultimatespins;
}
...
async function getSetpacks(packsa) {
    const res = await getPromise(`SELECT ${packsa} FROM packs;`);
    return res?.packsa;
}
...
async function addUltimateSpins(user, spins) {
    const res1 = await runPromise(`UPDATE users SET ultimatespins = ultimatespins + ${spins} WHERE id = (?);`, user);
    return res1.changes > 0;
}
async function removeUltimateSpins(user, spins) {
    const res1 = await runPromise(`UPDATE users SET ultimatespins = ultimatespins - ${spins} WHERE id = (?);`, user);
    return res1.changes > 0;
}
...
async function setSetpacks(type, packs) {
    return runPromise(`UPDATE packs SET ${type} = ${packs};`);  
}
